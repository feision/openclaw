...
### 🧠 记忆维护（新增 - Memory System v2.0）
- [ ] **每日**: 同步索引 - 运行 `python3 .memory/scripts/sync.py`
- [ ] **每周**: 清理过期记忆（>90天 + 重要性<0.5）
- [ ] **每周**: 合并相似记忆（相似度 > 0.8）
- [ ] **每次会话结束**: 保存会话摘要 `mm.save_session_summary()`
...
- **记忆维护**：每日/每周任务建议通过 cron 自动化（见 `scripts/` 目录）
```
